...
async def create_invitation(
    ...
    logger.info(f"초대장 생성 시도: 작가 UUID {artist_uuid[:8]}..., 전시 ID {data.exhibition_id}")
    ...
    if existing_invitation:
        logger.warning(f"중복 초대장 생성 시도: 작가 '{artist.name}', 전시 '{exhibition.title}'")
        raise HTTPException(
            status_code=409,
    ...
    logger.info(f"✅ 초대장 생성 완료: ID {invitation.id}, 코드 {invitation_code[:8]}..., 작가 '{artist.name}', 전시 '{exhibition.title}'")
...
async def get_invitation_detail(
    ...
    if invitation.artist_id != artist.id:
        logger.warning(f"권한 없음: 초대장 ID {invitation_id}, 요청 작가 '{artist.name}', 소유 작가 ID {invitation.artist_id}")
...
async def delete_invitation(
    ...
    logger.info(f"✅ 초대장 삭제 완료: ID {invitation_id}, 코드 {invitation_code[:8]}..., 작가 '{artist.name}'")
...
async def create_invitation_interest(
    ...
    logger.info(f"초대장 관심 표현 시도: UUID {user_uuid[:8]}..., 초대장 ID {data.invitation_id}")
    ...
    if artist and invitation.artist_id == artist.id:
        logger.warning(f"본인 전시 관심 표현 시도: 작가 '{artist.name}', 초대장 ID {data.invitation_id}")
        raise HTTPException(
            status_code=400,
    ...
    if existing.first():
        ...
        logger.warning(f"중복 관심 표현 시도: {user_type}, 초대장 ID {data.invitation_id}")
        raise HTTPException(
            status_code=409,
