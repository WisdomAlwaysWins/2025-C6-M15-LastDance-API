...
@router.post("", response_model=VisitHistoryResponse, status_code=status.HTTP_201_CREATED)
def create_visit_history(
    ...
    # Exhibition 존재 여부 확인
    exhibition = db.query(Exhibition).filter(Exhibition.id == visit_data.exhibition_id).first()
    if not exhibition:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
